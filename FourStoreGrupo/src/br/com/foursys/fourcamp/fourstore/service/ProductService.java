...
public class ProductService {
	...
	public boolean registerSale(Product product, Integer qtt, String cpf, String paymentType) {
			...
			if(verifyQtt != null && qtt<=product.getQtt()) {
				Integer qttNova = product.getQtt() - qtt;
				if(qttNova == 0) {
					data.delete(product);
				}else {
					product.setQtt(qttNova);
					data.update(product);
				}
				addSale(product, cpf, paymentType);
	...
	private void addSale(Product product, String cpf, String paymentType) {
